...
public interface ClientDashboardRepository extends AbstractRepository {
	...
	@Query("select avg(c.budget.amount) from Contract c where c.budget.currency = :currency and c.client.id = :id and c.draftMode = false")
	Double averageContractsBudget(int id, String currency);

	@Query("select stddev(c.budget.amount) from Contract c where c.budget.currency = :currency and c.client.id = :id and c.draftMode = false")
	Double deviationContractsBudget(int id, String currency);

	@Query("select min(c.budget.amount) from Contract c where c.budget.currency = :currency and c.client.id = :id and c.draftMode = false")
	Double minimumContractsBudget(int id, String currency);

	@Query("select max(c.budget.amount) from Contract c where c.budget.currency = :currency and c.client.id = :id and c.draftMode = false")
	Double maximumContractsBudget(int id, String currency);
